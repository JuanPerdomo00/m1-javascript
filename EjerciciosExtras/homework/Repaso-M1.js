...
LinkedList.prototype.changeNotNumbers = function () {
  // Tu código aca:
 // Head --> [1] --> ['2'] --> [false] --> ['Kiricocho']

 let counter = 0; // 1
 let current = this.head;

 while(current){
     if(isNaN(Number(current.value))){ // if(isNaN(NaN))
         counter++;
...
var mergeQueues = function (queueOne, queueTwo) {
  ...
  while (queueOne.size() || queueOne.size()) {
    let firstElement = queueOne.dequeue();
    let secondELement = queueTwo.dequeue();

    if (firstElement) quie.enqueue(firstElement);
    if (secondELement) quie.enqueue(secondELement);
  }
